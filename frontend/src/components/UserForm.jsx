    ...
    .superRefine((inductions, ctx) => {
      ...
      inductions.forEach((induction, index) => {
        ...
        if (induction.dueDate < induction.availableFrom) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "Due date must be greater than or equal to the available from date.",
            path: [index, "dueDate"],
          });
        }
      });
...
export const UserForm = ({ userData = DefaultNewUser, onSubmit }) => {
  ...
  useEffect(() => {
    setUser(userData);
    setAssignedInductions(Array.isArray(userData.assignedInductions) ? userData.assignedInductions : []);
  ...
  const handleRemoveInduction = (index) => {
    const updatedInductions = newAssignedInductions.filter((_, i) => i !== index);
    setNewAssignedInductions(updatedInductions);
    methods.setValue("newAssignedInductions", updatedInductions, {
      shouldValidate: true,
    });
  ...
  return (
    <>
            ...
            >
              ...
              {newAssignedInductions.map((induction, index) => (
                <div key={index} className="mb-4">
                    ...
                    <Select
                      ...
                      value={
                        availableInductions.find((ind) => ind.id === induction.id)
                          ? {
                              value: induction.id,
                              label: availableInductions.find((ind) => ind.id === induction.id)
                                .name,
                            }
                          : null
                      }
                  ...
                  {methods.formState.errors.newAssignedInductions?.[index]?.dueDate && (
                    <p className="text-red-500 text-xs italic mt-2">
                      {methods.formState.errors.newAssignedInductions[index].dueDate.message}
                    </p>
                  )}
                  {methods.formState.errors.newAssignedInductions?.[index]?.id && (
                    <p className="text-red-500 text-xs italic mt-2">
                      {methods.formState.errors.newAssignedInductions[index].id.message}
                    </p>
                  )}
                  {methods.formState.errors.newAssignedInductions?.[index]?.name && (
                    <p className="text-red-500 text-xs italic mt-2">
                      {methods.formState.errors.newAssignedInductions[index].name.message}
                    </p>
                  )}
                </div>
              ))}
      ...
      <div className="flex items-start justify-center pt-8">
      <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-4xl mx-4 md:mx-8">
        {userData.uid && assignedInductions.length > 0 && (
