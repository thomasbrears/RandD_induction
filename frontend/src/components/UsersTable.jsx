...
import {User, Mail, ArrowUpDown, Search, ChevronsLeft, ChevronLeft, ChevronRight, ChevronsRight} from "lucide-react";
...
const UsersTable = () => {
  ...
  const handleClick = async (uid) => {
    try {
      ...
      console.log("User data:", userData); // Print the user data to the console this will instead route to edit user with the correct data
    } catch (error) {
      ...
      <tbody>
        {users.map((user) => (
          <tr key={user.uid}>
            <td>{user.uid}</td>
            <td>{user.email}</td>
            <td><button onClick={() => handleClick(user.uid)}>
                  Get User Info
                </button></td>
          </tr>
        ))}
