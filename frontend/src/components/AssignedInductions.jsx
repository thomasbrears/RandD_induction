...
const AssignedInductions = ({uid}) => {
  ...
  useEffect(() => {
    if (user && uid) {
      const fetchInductions = async () => {
        try {
          const response = await getAssignedInductions(user, uid);
  ...
  /*const getStatus = (assignedInductions) => {
    const now = new Date();
    if (assignedInductions.completed) return 'Completed';
    if (new Date(assignedInductions.dueDate) < now) return 'Overdue';
    return 'To Do';
  };*/
